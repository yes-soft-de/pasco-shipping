...
import 'package:pasco_shipping/module_proxies/state_manger/proxies_state_manager.dart';
import 'package:pasco_shipping/module_proxies/ui/state/distributors_state/proxies_state.dart';
import 'package:pasco_shipping/module_proxies/ui/state/distributors_state/proxies_successfully.dart';
...
class ProxiesScreen extends StatefulWidget {
  final ProxyStateManager _stateManager;

  ProxiesScreen(this._stateManager);

  @override
  _CountriesScreenState createState() => _CountriesScreenState();
}

class _CountriesScreenState extends State<ProxiesScreen> {
  late ProxiesState currentState;
  late List<ProxyModel> items;
  @override
  ...
  void initState() {
    ...
    widget._stateManager.getProxies();
  }
  ...
  Widget Screen(){
    if(currentState is LoadingState){
      ...
      items = state!.proxies;
      return ProxiesSuccessfully(items: items ,onDelete: (id){
        widget._stateManager.deleteProxy(id.toString());
      },
        onEdit: (request){
          widget._stateManager.updateProxy(request);
        },
      );
    }
