...
class ContainerSpecificationRepository{
  ...
  Future<ConfirmResponse?> deleteSpecification(String id) async {
    // await _authService.refreshToken();
    var token = await _authService.getToken();
    var response = await _apiClient.delete(Urls.CONTAINER_SPECIFICATION+'/'+id,
        headers: {'Authorization': 'Bearer $token'});
    String? statusCode = ContainerSpecificationResponse.fromJson(response!).statusCode;
    String? msg = ContainerSpecificationResponse.fromJson(response).msg;
    if(statusCode =='401'){
  ...
  //
  Future<ConfirmResponse?> updateSpecification(ContainerSpecificationRequest request) async {
    // await _authService.refreshToken();
    var token = await _authService.getToken();

    var response = await _apiClient.put(Urls.CONTAINER_SPECIFICATION, request.toJson(),
        headers: {'Authorization': 'Bearer $token'});
    String? statusCode = ContainerSpecificationResponse.fromJson(response!).statusCode;
    String? msg = ContainerSpecificationResponse.fromJson(response).msg;
    if(statusCode =='204'){
