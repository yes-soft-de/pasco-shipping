...
class AirWaybillTravelDetailsSuccessfully extends StatefulWidget {
  ...
  const AirWaybillTravelDetailsSuccessfully(
      {required this.model,
      required this.onShipmentReview,
      required this.onUploadedToTravel,
      required this.onClearedOrArrived,
      required this.onShowFinance});

  @override
  _ContainerDetailsSuccessfullyState createState() =>
      _ContainerDetailsSuccessfullyState();
}

class _ContainerDetailsSuccessfullyState
    extends State<AirWaybillTravelDetailsSuccessfully> {
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text(
                  S.of(context).airwaybillInformation,
                  style: AppTextStyle.largeBlueBold,
                ),
                InkWell(
                    onTap: () {
          ...
          Padding(
            padding: const EdgeInsets.all(10.0),
            child: Text(
              S.of(context).shipmentInformation,
              style: AppTextStyle.largeBlueBold,
            ),
          ),
          ...
          (widget.model.shipmentID != null && widget.model.shipments!.isEmpty)
              ? (Container(
                  child: Padding(
                    padding: const EdgeInsets.all(10.0),
                    child: Text(
                      'this Air waybill requested for specific shipment',
                      style: AppTextStyle.mediumRed,
                    ),
                  ),
                ))
              ...
              ( widget.model.shippingStatus == TravelStatusName[TravelStatus.RELEASED] ||widget.model.shippingStatus == AcceptedShipmentStatusName[AcceptedShipmentStatus.CLEARED] || widget.model.shippingStatus == AcceptedShipmentStatusName[AcceptedShipmentStatus.ARRIVED]))
                      ...
                      : Container(
                          child: Padding(
                            padding: const EdgeInsets.all(10.0),
                            child: Text(
                              S.of(context).airwaybillUploaded,
                              style: AppTextStyle.mediumRed,
                            ),
                          ),
                        )
  ...
  Widget firstCard() {
    if (widget.model.status == AirwaybillStatusName[AirwaybillStatus.NOTFULL]) {
      return Padding(
        ...
        AirwaybillStatusName[AirwaybillStatus.FULL]) {
      return Padding(
        padding: const EdgeInsets.all(15.0),
        child: Card(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
  ...
  Widget uploadToTravel() {
    return Column(
      children: [
        RoundedButton(
            lable: S.of(context).uploadAirwaybill,
            icon: '',
            color: AppThemeDataService.AccentColor,
            style: AppTextStyle.mediumWhite,
            go: () {
              AddAirwaybillToTravelRequest re = AddAirwaybillToTravelRequest(
                  holderID: widget.model.id!, shipmentStatus: 'uploaded');
  ...
  Widget changeContainerStatus() {
    if (widget.model.shipments!.isNotEmpty &&
      ...
      return Padding(
        padding: const EdgeInsets.all(10.0),
        child: Text(
          S.of(context).airwaybillArrived,
