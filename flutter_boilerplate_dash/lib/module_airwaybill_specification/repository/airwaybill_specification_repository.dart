...
@injectable
class AirwaybillSpecificationRepository{
  final ApiClient _apiClient;
  final AuthService _authService;

  AirwaybillSpecificationRepository(this._apiClient, this._authService);
  ...
  Future<List<AirwaybillSpecificationModel>?> getAirwaybillSpecification() async {
    ...
    try {
      ...
      List<AirwaybillSpecificationModel>? marks =
          AirwaybillSpecificationResponse.fromJson(response!).data;
      return marks;
    } catch (_) {
      return null;
    }
  ...
  Future<ConfirmResponse?> createAirwaybillSpecification(AirwaybillSpecificationRequest request) async {
    ...
    var response = await _apiClient.post(Urls.AIRWABILL_SPECIFICATION, request.toJson(),
        headers: {'Authorization': 'Bearer $token'});
    String? statusCode = AirwaybillSpecificationResponse.fromJson(response!).statusCode;
    String? msg = AirwaybillSpecificationResponse.fromJson(response).msg;
    if(statusCode =='201'){
