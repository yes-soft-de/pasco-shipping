...
class ReceiverRepository{
  ...
  Future<List<ReceiverModel>?> getMyReceiver() async {
    var token = await _authService.getToken();
    try {
      var response = await _apiClient.get(Urls.MY_RECEIVER,
          headers: {'Authorization': 'Bearer $token'});
      ReceiverResponse markResponse =  ReceiverResponse.fromJson(response!);
      List<ReceiverModel>? marks = [];
      if(markResponse.data != null){
        marks =  ReceiverResponse.fromJson(response).data;
      }
      return marks;
    } catch (_) {
      return null;
  ...
  Future<ConfirmResponse?> createReceiver(ReceiverRequest request) async {
    ...
    var response = await _apiClient.post(Urls.RECEIVER, request.toJson(),
        headers: {'Authorization': 'Bearer $token'});
  ...
  Future<ConfirmResponse?> deleteReceiver(String id) async {
    // await _authService.refreshToken();
    var token = await _authService.getToken();
    var response = await _apiClient.delete(Urls.RECEIVER+'/'+id,
        headers: {'Authorization': 'Bearer $token'});
    String? statusCode = ReceiverResponse.fromJson(response!).statusCode;
    String? msg = ReceiverResponse.fromJson(response).msg;
    if(statusCode =='401'){
